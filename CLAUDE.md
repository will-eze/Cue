...
- **Two-tier ASR pipes** (`whisper-bin.js`): a `Map` of resident pipelines lets a fast interim model (tiny.en) stay loaded alongside the commit model. transcribe() **serializes per model** (onnxruntime CPU sessions aren't re-entrant) — keep that lock; concurrent decodes on one pipe corrupt output. Content matching is **lexical-first** (`lexical-index.js`, an in-memory inverted verse index — regenerable, never persisted) with MiniLM only as the paraphrase fallback; the lexical path runs on interims with no embedding index, so don't gate content detection on `contentMatch.isBuilt`.
