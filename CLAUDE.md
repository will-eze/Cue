...
- Broadcast-graphics overlay is an independent bus — never coupled to the program slide bus. Each slot (`nameTitle`/`ticker`/`custom`/`countdown`) holds **one occupant per destination kind** (`{screen, ndi}`) so a different graphic can run In-Room vs Online; each window gets `overlayForKind(kind)`. The operator UI must match "what's live" by the graphic's **`id`** carried in the slot, never by text content (two graphics sharing a body would both light up).
