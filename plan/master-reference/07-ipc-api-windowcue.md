...
- **Installer launch is platform-split**, then Cue quits ~1.2 s later (a running app can't be overwritten): **macOS** `shell.openPath(dmg)` just mounts the disk image for the user to drag to Applications. **Windows** `spawn(setup, ['/S'])` runs the NSIS installer **silently** (§20) — it reuses the existing install dir with no wizard and relaunches Cue itself. Do **not** `shell.openPath` the Windows Setup — that pops the full multi-page wizard on every update.
- This is "Option A" (manual one-click, plus the launch-time auto-check below). The Windows `/S` path makes updates effectively one-tap; true unattended background auto-update (Electron `autoUpdater`/`RELEASES` feed) is still not wired, and on macOS is gated by ad-hoc signing (needs a $99 Apple Developer ID + notarization). See `deployment-handoff.md` for signing.
